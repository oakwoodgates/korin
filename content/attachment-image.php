	...
	<?php if ( is_attachment() ) : // If viewing a single attachment. ?>
		...
		<header class="entry-header">
			...
			<div class="entry-byline">
				<span class="image-sizes"><?php printf( esc_html__( 'Sizes: %s', 'korin' ), hybrid_get_image_size_links() ); ?></span>
			</div><!-- .entry-byline -->
...
<?php if ( is_attachment() ) : // If viewing a single attachment. ?>
	...
	<div class="attachment-meta">
		...
		<div class="media-info image-info">
			...
			<h3 class="attachment-meta-title"><?php esc_html_e( 'Image Info', 'korin' ); ?></h3>

			<ul class="media-meta">
				<?php $pre = '<li><span class="prep">%s</span>'; ?>
				<?php hybrid_media_meta( 'dimensions',        array( 'before' => sprintf( $pre, esc_html__( 'Dimensions',    'korin' ) ), 'after' => '</li>' ) ); ?>
				<?php hybrid_media_meta( 'created_timestamp', array( 'before' => sprintf( $pre, esc_html__( 'Date',          'korin' ) ), 'after' => '</li>' ) ); ?>
				<?php hybrid_media_meta( 'camera',            array( 'before' => sprintf( $pre, esc_html__( 'Camera',        'korin' ) ), 'after' => '</li>' ) ); ?>
				<?php hybrid_media_meta( 'aperture',          array( 'before' => sprintf( $pre, esc_html__( 'Aperture',      'korin' ) ), 'after' => '</li>' ) ); ?>
				<?php hybrid_media_meta( 'focal_length',      array( 'before' => sprintf( $pre, esc_html__( 'Focal Length',  'korin' ) ), 'after' => '</li>', 'text' => esc_html__( '%s mm', 'korin' ) ) ); ?>
				<?php hybrid_media_meta( 'iso',               array( 'before' => sprintf( $pre, esc_html__( 'ISO',           'korin' ) ), 'after' => '</li>' ) ); ?>
				<?php hybrid_media_meta( 'shutter_speed',     array( 'before' => sprintf( $pre, esc_html__( 'Shutter Speed', 'korin' ) ), 'after' => '</li>', 'text' => esc_html__( '%s sec', 'korin' ) ) ); ?>
		...
		<?php if ( $gallery ) : // Check if the gallery is not empty. ?>
			...
			<div class="image-gallery">
				<h3 class="attachment-meta-title"><?php esc_html_e( 'Gallery', 'korin' ); ?></h3>
				<?php echo $gallery; ?>
			</div>
